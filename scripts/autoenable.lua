...
AddClassPostConstruct("screens/redux/modsscreen", function(self, ...)
	...
	local tableexists_mim = type(bettercrashscr_mimenabledmods) == "table"
		and next(bettercrashscr_mimenabledmods) ~= nil

	local TEMPLATES = require("widgets/redux/templates")

	if GetModConfigData("savemodspos") == 0 then
		self.saveperistentmods_big = self.optionspanel:AddChild(
			TEMPLATES.StandardButton(
				nil,
				tableexists == true
						and STRINGS.UI.MAINSCREEN.BETTERCRASHSCREEN[TUNING.BETTECRASHSCREEN_LANGUAGE].SAVEPERISTENTMODS_CLEAR
					or STRINGS.UI.MAINSCREEN.BETTERCRASHSCREEN[TUNING.BETTECRASHSCREEN_LANGUAGE].SAVEPERISTENTMODS_SAVE
			)
		)
		self.saveperistentmods_big:SetScale(0.7)
		...
		self.saveperistentmods_big.onclick = function()
			...
			tableexists_mim = type(bettercrashscr_mimenabledmods) == "table"
				and next(bettercrashscr_mimenabledmods) ~= nil
			...
			local locationData = {
				bettercrashscr_enabledmods = tableexists and {} or mymods,
				bettercrashscr_mimenabledmods = tableexists_mim and {} or mymods_mim,
			}
			local jsonString = json.encode(locationData)
			TheSim:SetPersistentString("BetterCrashScreen_EnabledMods", jsonString, false, function()
				self.saveperistentmods_big.text:SetString(
					tableexists == true
							and STRINGS.UI.MAINSCREEN.BETTERCRASHSCREEN[TUNING.BETTECRASHSCREEN_LANGUAGE].SAVEPERISTENTMODS_SAVE
						or STRINGS.UI.MAINSCREEN.BETTERCRASHSCREEN[TUNING.BETTECRASHSCREEN_LANGUAGE].SAVEPERISTENTMODS_CLEAR
				)
			end)
		end
	elseif GetModConfigData("savemodspos") == 1 then
		self.saveperistentmods = self.root:AddChild(
			TEMPLATES.IconButton(
				...
				function()
					HeyVSauceMichaelHere()
					tableexists = type(bettercrashscr_enabledmods) == "table"
						and next(bettercrashscr_enabledmods) ~= nil
					tableexists_mim = type(bettercrashscr_mimenabledmods) == "table"
						and next(bettercrashscr_mimenabledmods) ~= nil
					...
					local jsonString = json.encode(locationData)
					TheSim:SetPersistentString("BetterCrashScreen_EnabledMods", jsonString, false, function()
						self.saveperistentmods.text:SetString(
							tableexists == true
									and STRINGS.UI.MAINSCREEN.BETTERCRASHSCREEN[TUNING.BETTECRASHSCREEN_LANGUAGE].SAVEPERISTENTMODS_SAVE
								or STRINGS.UI.MAINSCREEN.BETTERCRASHSCREEN[TUNING.BETTECRASHSCREEN_LANGUAGE].SAVEPERISTENTMODS_CLEAR
						)
						self.saveperistentmods.text_shadow:SetString(
							tableexists == true
									and STRINGS.UI.MAINSCREEN.BETTERCRASHSCREEN[TUNING.BETTECRASHSCREEN_LANGUAGE].SAVEPERISTENTMODS_SAVE
								or STRINGS.UI.MAINSCREEN.BETTERCRASHSCREEN[TUNING.BETTECRASHSCREEN_LANGUAGE].SAVEPERISTENTMODS_CLEAR
						)
	...
	elseif GetModConfigData("savemodspos") == 2 then
		local offset = Vector3(0, 0, 0)
		for k, v in pairs(self.mods_page.allmodsmenu) do
			offset = Vector3(#k, 0, 0)
		end

		if self.saveperistentmods == nil then
			self.saveperistentmods = self.mods_page.allmodsmenu:AddCustomItem(
				TEMPLATES.IconButton(
					"images/button_icons.xml",
					"save.tex",
					tableexists == true
					...
					function()
						...
						local jsonString = json.encode(locationData)
						TheSim:SetPersistentString("BetterCrashScreen_EnabledMods", jsonString, false, function()
							self.saveperistentmods:SetHoverText(
								tableexists == true
										and STRINGS.UI.MAINSCREEN.BETTERCRASHSCREEN[TUNING.BETTECRASHSCREEN_LANGUAGE].SAVEPERISTENTMODS_SAVE
									or STRINGS.UI.MAINSCREEN.BETTERCRASHSCREEN[TUNING.BETTECRASHSCREEN_LANGUAGE].SAVEPERISTENTMODS_CLEAR
							)
							if tableexists == true then
								self.saveperistentmods.icon:SetTint(unpack(RGB(255, 50, 0)))
							else
								self.saveperistentmods.icon:SetTint(unpack(RGB(0, 255, 50)))
							end
				...
				offset -- pass the Vector3 offset here
			)
		end
		...
		--self.saveperistentmods:SetPosition(0, 0)
		self.saveperistentmods:SetTextSize(15)
		self.saveperistentmods.text:SetPosition(TEXT_OFFSET, 0)
		--self.saveperistentmods.text_shadow:SetPosition(TEXT_OFFSET + 2, 0)
		self.saveperistentmods:SetScale(1)
		self.saveperistentmods.text:SetHAlign(ANCHOR_MIDDLE)
		--self.saveperistentmods.text_shadow:SetHAlign(ANCHOR_MIDDLE)
...
AddClassPostConstruct("screens/redux/mainscreen", function(self, ...)
	function CheckForDisabledMods()
		...
		print(tableexists)
		if not tableexists or (is_mim_enabled and not tableexists_mim) then
			return
		end
		...
		if
			(not isenabled and GetModConfigData("autoapply") == 1)
			or (GetModConfigData("autoapply_mim") == 1 and (is_mim_enabled and not isenabled_mim))
		then
			EnableDemMods(v)
		...
		if not isenabled or (is_mim_enabled and not isenabled_mim) then
			local dialogue = PopupDialogScreenRedux(
				(is_mim_enabled and not isenabled_mim)
						and STRINGS.UI.MAINSCREEN.BETTERCRASHSCREEN[TUNING.BETTECRASHSCREEN_LANGUAGE].TITLE_MIMMODSDISABLED
					or STRINGS.UI.MAINSCREEN.BETTERCRASHSCREEN[TUNING.BETTECRASHSCREEN_LANGUAGE].TITLE_NORMALMODSDISABLED,
				(is_mim_enabled and not isenabled_mim)
						and STRINGS.UI.MAINSCREEN.BETTERCRASHSCREEN[TUNING.BETTECRASHSCREEN_LANGUAGE].MIMMODSDISABLED
					or STRINGS.UI.MAINSCREEN.BETTERCRASHSCREEN[TUNING.BETTECRASHSCREEN_LANGUAGE].NORMALMODSDISABLED,
				{
					{
